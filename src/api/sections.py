...
from src.db.models.course import Section, SectionPublic, ContentBlock, ContentBlockPublic, SectionCreate, SectionUpdate, ContentBlockCreate, ContentBlockUpdate
...
async def read_section(id: int, session: SessionDep):
    """Retrieve a section by ID. Raises 404 if not found."""
...
async def update_section(id: int, section: SectionUpdate, session: SessionDep):
    """Update a section by ID. Raises 404 if not found."""
...
async def delete_section(id: int, session: SessionDep):
    """Delete a section by ID. Raises 404 if not found."""
    section = session.get(Section, id)
    if not section:
        raise HTTPException(status_code=404, detail="Section not found")
...
async def read_section_content_blocks(id: int, session: SessionDep):
    """Retrieve all content blocks for a section by ID. Raises 404 if not found."""
    section = session.get(Section, id)
    if not section:
        raise HTTPException(status_code=404, detail="Section not found")
...
async def read_content_block(id: int, session: SessionDep):
    """Retrieve a content block by ID. Raises 404 if not found."""
...
async def update_content_block(id: int, content_block: ContentBlockUpdate, session: SessionDep):
    """Update a content block by ID. Raises 404 if not found."""
...
async def delete_content_block(id: int, session: SessionDep):
    """Delete a content block by ID. Raises 404 if not found."""
    content_block = session.get(ContentBlock, id)
    if not content_block:
        raise HTTPException(status_code=404, detail="Content block not found")
